...
export async function uploadFile(payload: UploadForm): Promise<TaskResponse> {
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/8ba094c0-f913-4a1d-9d69-0a38a5483749',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'uploadFileApi.ts:6',message:'API call starting',data:{endpoint:'/api/v1/task/parse',fileName:payload.file_name,segmentationStrategy:payload.segmentation_strategy,pipeline:payload.pipeline,fullPayload:JSON.stringify(payload)},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'H12-H13'})}).catch(()=>{});
  // #endregion
