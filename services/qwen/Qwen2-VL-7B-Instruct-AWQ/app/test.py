...
def process_images(messages_batch: List[List[Dict[str, Any]]], image_files: List[str]):
    ...
    # Retain image references in messages to allow the server to associate images.
    ...
    print(f"Number of image references in messages: {sum(
        1 for batch in messages_batch for msg in batch if msg.get("role") == "user" and any(c.get("type") == "image" for c in msg.get("content", []))
    )}")
...
def test_single_request():
    ...
    image_file = os.path.join(test_dir, "image1.png")
