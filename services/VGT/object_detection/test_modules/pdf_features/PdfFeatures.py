...
class PdfFeatures:
    ...
    def from_pdf_path(pdf_path, xml_path: str = None):
        print(f"Converting PDF at path: {pdf_path}")
        ...
        print("Running initial pdftohtml conversion...")
        ...
        if pdf_features is None:
            print(f"Failed to extract features from PDF!!!: {pdf_path}")
            return None

        if remove_xml and exists(xml_path):
            print(f"Cleaning up temporary XML file: {xml_path}")
            os.remove(xml_path)
