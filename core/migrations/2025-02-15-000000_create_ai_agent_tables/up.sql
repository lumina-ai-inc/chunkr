...
CREATE TABLE IF NOT EXISTS investor_memos (
...
ADD COLUMN IF NOT EXISTS fact_extraction_status VARCHAR(50) DEFAULT 'pending', -- 'pending' | 'processing' | 'completed' | 'failed'
ADD COLUMN IF NOT EXISTS fact_extraction_completed_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS embedding_id VARCHAR(255); -- reference to vector DB
...
ALTER TABLE facts
ADD COLUMN IF NOT EXISTS confidence_score FLOAT CHECK (confidence_score >= 0.0 AND confidence_score <= 1.0), -- 0.0 to 1.0
ADD COLUMN IF NOT EXISTS extraction_method VARCHAR(50) DEFAULT 'ai', -- 'ai' | 'manual' | 'rule_based'
ADD COLUMN IF NOT EXISTS reviewed_by_user BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS embedding_id VARCHAR(255); -- reference to vector DB for semantic search
