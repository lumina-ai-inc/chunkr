...
DECLARE
  ...
  overage_pages INT;
  overage_rate  FLOAT8;
  v_bill_date   DATE; -- Renamed variable
  inv_id        TEXT;
BEGIN
  IF TG_OP = 'UPDATE'
  ...
  THEN
    ...
    IF total_usage > rec_cycle.usage_limit THEN
      ...
      SELECT tiers.overage_rate INTO overage_rate -- Explicitly qualify column
        FROM tiers
        WHERE tier = rec_cycle.tier;
      v_bill_date := rec_cycle.billing_cycle_end::DATE; -- Assign to renamed variable
      ...
      WHERE user_id       = NEW.user_id
        ...
        AND DATE(invoices.bill_date) = v_bill_date -- Use renamed variable
      LIMIT 1;
      ...
      IF NOT FOUND THEN
        ...
        INSERT INTO invoices (
          ...
          overage_pages,
          NOW(),
          v_bill_date -- Use renamed variable value for insert
        );
      ELSE
        UPDATE invoices
        SET
