...
use crate::models::pipeline::{validate_and_convert_identified_elements, Element, Pipeline};
...
pub async fn process(
    ...
    let identification_futures = sheets
        .par_iter()
        .enumerate()
        .map(|(index, sheet)| {
            ...
            if segmentation_strategy == SegmentationStrategy::Page {
                ...
                let result: Result<Vec<Element>, Box<dyn Error + Send + Sync>> = Ok(
                    sheet.sheet_info.clone().into()
                );
                return futures::future::ready(result).boxed();
            }
            ...
            (
                async move {
                    ...
                    span.set_attribute(
                        opentelemetry::KeyValue::new("sheet_name", sheet.sheet_info.name.clone())
                    );
                    ...
                    let html_content = match fs::read_to_string(html_file.path()) {
                        ...
                        Err(e) => {
                            ...
                            sheet_context
                                .span()
                                .set_attribute(
                                    opentelemetry::KeyValue::new("error", error_msg.clone())
                                );
                    ...
                    let image_url = match
                        get_file_url(&sheet.sheet_capture_with_headers.image, &s3_location).await
                    {
                        Ok(url) => url,
                        Err(e) => {
                            let error_msg = e.to_string();
                    ...
                    let result = match
                        structured_output_from_template::<IdentifiedElements>(
                        ...
                        ).await
                    {
                        Ok(identified_elements) => {
                            // Validate and truncate ranges to ensure they're within sheet bounds
                            match
                                validate_and_convert_identified_elements(
                                    identified_elements,
                                    &sheet.sheet_info
                                )
                            {
                                Ok(validated_elements) => { Ok(validated_elements) }
                                Err(e) => {
                                    println!(
                                        "Failed to validate ranges for sheet {}: {}",
                                        sheet.sheet_info.name,
                                        e
                                    );
                                    // Fall back to using the sheet as a single element if validation fails
                        ...
                        Err(e) => {
                            ...
                            if
                                let Some(LLMError::NonRetryable(_error_msg)) =
                                    e.downcast_ref::<LLMError>()
                            {
                                println!(
                                    "Skipping table identification for sheet {index} due to non-retryable client error: {e}"
                                );
                                sheet_context
                                    .span()
                                    .set_attribute(
                            ...
                            if error_handling == ErrorHandlingStrategy::Fail {
                                ...
                                sheet_context
                                    .span()
                                    .set_status(
                                        opentelemetry::trace::Status::error(error_msg.clone())
                                    );
                                sheet_context.span().record_error(e.as_ref());
                                sheet_context
                                    .span()
