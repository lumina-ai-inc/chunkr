...
use crate::utils::rate_limit::{create_llm_rate_limiter, RateLimiter};
...
/// TODO: Should retries be here? Can the LLM response change?
pub async fn html_ocr(
    temp_file: &NamedTempFile,
    prompt: String,
) -> Result<String, Box<dyn Error + Send + Sync>> {
    let response = llm_ocr(temp_file, prompt).await?;
    let html = extract_fenced_content(&response, "html")
...
pub async fn markdown_ocr(
    ...
    let response = llm_ocr(temp_file, prompt).await?;
    let markdown = extract_fenced_content(&response, "markdown")
        .ok_or_else(|| "No markdown content found in table OCR result")?;
    Ok(markdown)
}
...
pub async fn latex_ocr(
    ...
    let response = llm_ocr(temp_file, prompt).await?;
    let latex = extract_fenced_content(&response, "latex")
        .ok_or_else(|| "No latex content found in table OCR result")?;
    Ok(latex)
}
...
//                         let messages = match get_basic_image_message(&temp_file, prompt) {
