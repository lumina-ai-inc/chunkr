...
pub fn get_mkd_from_llm_table_ocr(
    table_ocr_result: String,
) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    if let Some(mkd_content) = table_ocr_result.split("```markdown").nth(1) {
        if let Some(mkd) = mkd_content.split("```").next() {
            return Ok(mkd.trim().to_string());
        }
    }
    Err("No Markdown content found in table OCR result".into())
}
