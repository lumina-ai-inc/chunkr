-- Your SQL goes here
-- Add primary key to pre_applied_free_pages table
ALTER TABLE pre_applied_free_pages 
ADD COLUMN IF NOT EXISTS id SERIAL PRIMARY KEY;

-- Insert new usage types into USAGE_TYPE table with default values
INSERT INTO USAGE_TYPE (id, type, description, unit, cost_per_unit_dollars)
VALUES 
    ('NoModel', 'NoModel', 'No model processing', 'page', 0.01),
    ('NotKnown', 'NotKnown', 'Unknown model processing', 'page', 0.01)
ON CONFLICT (id) DO NOTHING;

-- Update update_usage_on_success function to handle new usage types without raising exceptions
...
BEGIN
    IF TG_OP = 'UPDATE' AND NEW.status = 'Succeeded' THEN
        ...
        ELSIF v_config->>'model' = 'NotKnown' THEN
...
-- Update update_monthly_usage function to handle new usage types without raising exceptions
...
-- Update handle_task_invoice function to handle new usage types without raising exceptions
...
DECLARE
    ...
    v_invoice_id TEXT;
    v_cost_per_unit FLOAT DEFAULT 0.01;
    v_cost FLOAT;
    v_config JSONB;
    v_current_month INTEGER;
...
BEGIN
    ...
    IF NEW.status = 'Succeeded' THEN
        ...
        SELECT COALESCE(cost_per_unit_dollars, 0.01) INTO v_cost_per_unit
        FROM USAGE_TYPE
