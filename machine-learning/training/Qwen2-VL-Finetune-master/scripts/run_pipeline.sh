...
OUTPUT_DIR=${OUTPUT_DIR:-"output/sophris_table_vllm_compat_7b"} # Unified output directory for all runs
...
NUM_EPOCHS=${NUM_EPOCHS:-1} # Total desired epochs (adjust if resuming)
...
TRAIN_ARGS=(
    ...
    "--per_device_train_batch_size" "1"  # Reduced from $BATCH_PER_DEVICE
    "--gradient_accumulation_steps" "8"  # Increased from $GRAD_ACCUM_STEPS
    ...
    "--dataloader_num_workers" "2"  # Reduced from previous value
...
HUB_MODEL_ID=${HUB_MODEL_ID:-"ChunkrAI/chunkr-table-v1-qwen2_5VL-7B"}
VISUALIZATION_DIR=${VISUALIZATION_DIR:-"$OUTPUT_DIR/visualizations"}
NUM_EPOCHS=${NUM_EPOCHS:-2} # Total desired epochs (adjust if resuming)
