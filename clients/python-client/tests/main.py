...
async def main():
    ...
    company = Company(
        name="Example Corp", industry=Industry.TECHNOLOGY, address=address
    )

    department = Department(name="Engineering", head_count=50, location=address)
...
class TermEnrollment(BaseModel):
    ...
    portland: CampusEnrollment = Field(
        description="Enrollment numbers for Portland campus"
    )
...
class EnrollmentReport(BaseModel):
    ...
    enrollments: List[TermEnrollment] = Field(
        description="List of enrollment data by term"
    )
...
async def test_enrollment_schema():
    ...
    report = EnrollmentReport(
        ...
        enrollments=[
            TermEnrollment(
                term="Fall 2019",
                eugene=CampusEnrollment(undergraduate=19886, graduate=3441),
                portland=CampusEnrollment(undergraduate=1024, graduate=208),
            ),
            TermEnrollment(
                term="Winter 2020",
                eugene=CampusEnrollment(undergraduate=19660, graduate=3499),
                portland=CampusEnrollment(undergraduate=1026, graduate=200),
            ),
            TermEnrollment(
                term="Spring 2020",
                eugene=CampusEnrollment(undergraduate=19593, graduate=3520),
                portland=CampusEnrollment(undergraduate=998, graduate=211),
            ),
        ],
    )
    ...
    # Upload to Chunkr
    task = await chunkr.upload(
        "../../tests/files/test.pdf", Configuration(json_schema=instance_schema)
    )
    await task.poll()
