...
/// PictureCroppingStrategy : Controls the cropping strategy for an item (e.g. segment, chunk, etc.) - `All` crops all images in the item - `Auto` crops images only if required for post-processing
...
impl std::fmt::Display for PictureCroppingStrategy {
...
impl Default for PictureCroppingStrategy {
    fn default() -> PictureCroppingStrategy {
        Self::All
    }
}
